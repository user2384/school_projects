 ...
 ** MAX_QUEUE. Hodnota QUEUE_SIZE slouží k simulaci fronty v různe velkém poli
...
void queueInit (tQueue* q) {
    ...
    while (cnt < QUEUE_SIZE) //counter prejde celé  pole
    {
        q->arr[cnt] = '*'; //hodnotu každého prvku v poli nastaví na '*'
        cnt++;
    }
    q->f_index = 0; //index začiatku nastaví na 0
    q->b_index = 0; //index prvého voľného miesta nastaví na 0
}
...
int nextIndex (int index) {
    ...
    return ((index + 1) % QUEUE_SIZE); //vráti sa index začiatočného prvku v prípade, že index presiahne QUEUE_SIZE
    
}

int queueEmpty (const tQueue* q) {
    /*
     ** Vrací nenulovou hodnotu, pokud je frona prázdná, jinak vracíhodnotu 0.
     ** Funkci je vhodné implementovat jedním příkazem return.
     */
    
    return (q->f_index == q->b_index); //vracia 1 ak oba indexy ukazujú na rovnaký prvok, tj. fronta je prázdna
}
...
int queueFull (const tQueue* q) {
    ...
    return (nextIndex(q->b_index) == q->f_index); //vracia 1 v prípade, že index za začiatkom fronty a index prvého voľného prvku sa zhodujú
}
...
void queueFront (const tQueue* q, char* c) {
    ...
    if (queueEmpty(q)) //v prípade, že je fronta prázdna
    {
        queueError(QERR_FRONT);
        return;
    }
    (*c) = (q->arr[q->f_index]); //na adresu kam ukazuje parameter c uloží znak zo začiatku fronty
}
...
void queueRemove (tQueue* q) {
    ...
    q->f_index = nextIndex(q->f_index); //prvok za aktuálnym f->index sa stáva indexom prvého prvku
}
...
void queueGet (tQueue* q, char* c) {
    ...
    queueFront(q, c); //na prvý znak fronty bude ukazovať c
...
void queueUp (tQueue* q, char c) {
    ...
    if (queueFull(q)) //v prípade, že je fronta plná
    {
        queueError(QERR_UP);
        return;
    }
    q->arr[q->b_index] = c; //na prvý voľný index vo fronte vloží znak c
    q->b_index = nextIndex(q->b_index); //index prvého voľného prvku sa posunie na nasledujúci prvok
}
